...
def getForwardHeaders(request):
    ...
    try:
        xqs_h = request.headers.get('x-qotm-session')
        app.logger.info("header xqs_h: %s" % (xqs_h,))
        #print "xqs_h: "+xqs_h
    except Exception, e:
        app.logger.info("Exception while get x-qotm-session header': %s" % (e,))
    try:
        via_h = request.headers.get('via')
        app.logger.info("header via_h: %s" % (via_h,))
        #print "via_h: "+via_h
    except Exception, e:
        app.logger.info("Exception while get via header: %s" % (e,))
    ...
    for ihdr in incoming_headers:
        ...
        if val is not None:
            headers[ihdr] = val
            #app.logger.info("header ihdr: %s, val: %s" % (ihdr, val))
