...
dispatcher.onGet(/^\/hratings\/[0-9]*/, function (req, res) {
    ...
    if (process.env.DB_TYPE === 'mysql') {
      ...
      connection.query('SELECT Rating FROM ratings', function (err, results, fields) {
        if (err) {
          ...
          res.end(JSON.stringify({error: 'could not connect to hratings database'}))
        } else {
          if (results[0]) {
      ...
      MongoClient.connect(url, function (err, db) {
        if (err) {
          ...
          db.collection('ratings').find({}).toArray(function (err, data) {
            if (err) {
              res.writeHead(500, {'Content-type': 'application/json'})
              res.end(JSON.stringify({error: 'could not load hratings from database'}))
            } else {
