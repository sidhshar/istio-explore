...
cd /home/sidhshar/repo/istio-explore/employee_v1/src/reviews
...
cd /home/sidhshar/repo/istio/samples/bookinfo/src/reviews
docker run --rm -v `pwd`:/usr/bin/app:rw niaquinto/gradle clean build
cd reviews-wlpcfg
...
docker run --rm -v `pwd`:/usr/bin/app:rw niaquinto/gradle clean build
cd reviews-wlpcfg
docker build -t reviews-v3 --build-arg service_version=v3 --build-arg enable_ratings=true --build-arg star_color=red .
